...
both arms uniformly). All three are evaluated on held-out.

**SELF-CHECK, amended 2026-07-29 before any arm was run.** This document originally required that
the 50,000 record equal the ES-returned best GENOME. That is unimplementable, and asserting it raised
on the first seed of the first arm.

The two sides break ties differently over the same landscape and cannot be made to agree. The
collector keeps the first genome reaching the maximum across EVERY evaluation, because `better/3`
holds the incumbent when `F =< BF`. `sep_cma_es` keeps the first across GENERATION TOPS only,
comparing `TopF > element(2, Best)` once per generation. The ladder fitness is a coarse count over
six training starts, so ties at the maximum are common, and on a tie the two legitimately hold
different genomes of equal fitness.

The self-check is therefore **equality of FITNESS**, which is the invariant that actually carries the
meaning, and it is strict in both directions: collector below optimiser means the collector missed
the winner, collector above means the optimiser did, and either way the seed is discarded. The
original wording was written before the optimiser's tie-breaking was read, and is corrected rather
than quietly satisfied by a weaker test.
